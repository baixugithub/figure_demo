...
u8 modeflag,clearallflag = 0,changeflag=0;//默认为识别模式，如果为1为录入指纹模式,每录入一次SaveNumber++
...
void shownum(unsigned char dat)
{
	...
	numdat=dat;
	LCD_ShowString(10,100,200,24,24,"Finger: ");
	str[0]=0x30+numdat/10;
	str[1]=0x30+numdat%10;
	LCD_ShowString(100,100,200,24,24,str);
	LCD_ShowString(125,100,200,24,24,"               ");
...
void mode(void)
{
	 if(modeflag==0)
	 {
		LCD_ShowString(10,130,200,24,24,"Lock:  Idenfity");
		green=0;
		red=1;
	 }
	 if(modeflag==1)
	 {
		LCD_ShowString(10,130,200,24,24,"Lock:  Input   ");
...
void FigureClear(void)
{
			...
			//蜂鸣器长响一次，表示清除所有指纹结束
//			modeflag=1;//进入录入指纹模式
			LCD_ShowString(10,130,200,24,24,"Clear  OK!        ");
//			LCD_ShowString(100,100,200,24,24," 0");
//			LCD_ShowString(10,100,200,24,24,"Finger:");
